    ...
    public class ExcelService : IExcelService
    {
                                      ...
                                      int dayCountInMonth, List<string> dayRangeInMonth, List<PaySlipModel> paySlips)
        {
            ...
            using (var package = new ExcelPackage())
            {
                ...
                worksheet.Cells[6, 4].Value = "Position";
                worksheet.Cells[6, 5].Value = "Basic salary";
                worksheet.Cells[6, 6].Value = "Overtime hours";
                ...
                worksheet.Cells["A6:F6"].Style.Fill.PatternType = ExcelFillStyle.Solid;
                worksheet.Cells["A6:E6"].Style.Fill.BackgroundColor.SetColor(blueColor);
                worksheet.Cells["F6:F6"].Style.Fill.BackgroundColor.SetColor(orangeColor);

                var endColumn = dayCountInMonth + 6;
                var columnChar = GetExcelColumnName(endColumn);
                var cellG6 = worksheet.Cells[$"G6:{columnChar}6"];

                for (int i = 0; i < dayRangeInMonth.Count; i++)
                {
                    worksheet.Cells[6, i + 7].Value = dayRangeInMonth[i];
                }
                ...
                for (int i = 0; i < filteredPaySlips.Count; i++)
                {
                    ...
                    worksheet.Cells[row, 4].Value = paySlip.Position;
                    worksheet.Cells[row, 5].Value = paySlip.BasicSalary;
                    worksheet.Cells[row, 6].Value = 0.00;
                    worksheet.Cells[row, 6].Style.Numberformat.Format = "0.00";
                    worksheet.Cells[row, 6].Style.Locked = false;
                ...
                worksheet.View.FreezePanes(6, 7);
        ...
        public ImportPaySlipModel ImportExcelFile(IFormFile file)
        {
            ...
            using (var package = new ExcelPackage(file.OpenReadStream()))
            {
                ...
                for (int i = 7; i <= rows; i++)
                {
                    ...
                    paySlip.Position = worksheet.Cells[i, 4].Value.ToString();
                    paySlip.BasicSalary = decimal.Parse(worksheet.Cells[i, 5].Value.ToString() ?? "");
                    paySlip.TotalOvertimeHours = int.Parse(worksheet.Cells[i, 6].Value.ToString() ?? "");
